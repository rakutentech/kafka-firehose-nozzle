...
func TestJsonEncoder_Encode_NoExtraContent(t *testing.T) {
	...
	encoder := toJSON(enrich(logMessage("hello", testAppId, timestamp), ""))
...
func TestJsonEncoder_Encode_ExtraContent(t *testing.T) {
	timestamp := int64(1461318380946558204)
	expect := fmt.Sprintf(
		`{"origin":"fake-origin-1","eventType":5,"timestamp":%d,"logMessage":{"message":"aGVsbG8=","message_type":1,"timestamp":1461318380946558204,"app_id":"%[2]s","source_type":"DEA"},"app_guid":"%[2]s"}`,
		timestamp, testAppId)

	encoder := toJSON(enrich(logMessage("hello", testAppId, timestamp), testAppId))
